...
`CCDASH_INSTALL_DIR=/custom/path` で配置先を上書き、`CCDASH_VERSION=v0.1.x` で特定バージョン固定 (GitHub の匿名 API レート制限に当たった時のフォールバック) も可能。

Claude Code に hook を組み込むのは 1 度だけ:
...
左ペインがセッション一覧、右ペインが選択中セッションのライブ transcript。タブストリップで repo / operator 命名タブごとに絞り込み。`All` は全件。
...
アクティブタブがアーカイブ等で消えると自動で次のタブへ移ります。

起動時にタブ固定:

```sh
ccdash --tab home-lab          # ストリップ非表示・固定
ccdash --tab "deploy review"   # スペース含む user_tab も OK
```
...
長く読みたい時は `o` でモーダルビューアに切替。tail-read ではなく**全文ロード**します。

---

## 6. 承認 (Approvals)

デフォルトで `PermissionRequest` hook を最大 25 秒ブロック。Claude の標準プロンプトにフォールバックされる前に TUI で判断できます。
...
`A` (keep-allow) は `updatedPermissions` を `scope: "session"` で返すので、同等の呼び出しは以降プロンプト無し。`Bash` の場合は最初の token で glob 化 (`Bash(git status)` → `Bash(git *)`)。

### タブ一括アーカイブ

`Ctrl+X` で現在のタブの全セッションを一括アーカイブ。条件:

1. アクティブフィルタが具体タブ (`All` 不可、誤爆対策)
...
`s` で選択中セッションの会話を Claude に要約させます。ccdash が:
...
`/` でフッター検索入力を開く。`Enter` で submit すると以下のフィールドに対して大文字小文字無視の部分一致でフィルタ:
