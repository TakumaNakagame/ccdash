...
const Banner = "[ccdash] attached — Ctrl+D detach (claude keeps running)\r\n"

// Result carries why a single Attach call returned. Detached is true on
// a clean Ctrl+D; ExitErr is set when the child died on its own.
type Result struct {
...
func (s *Session) Attach() (Result, error) {
	...
	// Stdin → PTY with Ctrl+D interception. Uses unix.Poll so the
